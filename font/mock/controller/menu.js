    ...
    children: [
      {
        path: 'menu',
        name: 'Menu',
        component: '@/views/systemManagement/menuManagement/index',
        meta: {
          title: '菜单管理',
        },
      },
      {
        path: 'user',
        name: 'User',
        component: '@/views/systemManagement/userManagement213123/index',
        meta: { title: '用户管理' },
      },
    ],
  },
...
module.exports = [
  {
    ...
    type: 'post',
    response(config) {
      const { kw = '', pageNo = 1, pageSize = 20 } = config.body
      const pageList = data.filter(
        (item, index) =>
